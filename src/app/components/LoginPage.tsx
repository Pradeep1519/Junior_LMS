// ============================================
// FINAL: student-portal/src/app/components/LoginPage.tsx
// ============================================
import { useState } from "react";
import { useNavigate } from "react-router";
import { Mail, Lock, Eye, EyeOff, ArrowRight, Sparkles } from "lucide-react";
import { loginWithEmail, getUserData, db } from "../config/firebase";
...
export function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // ✅ Check teachers collection - agar teacher hai to block
      const teacherDocRef = doc(db, "teachers", result.user.uid);
      const teacherDocSnap = await getDoc(teacherDocRef);
      
      if (teacherDocSnap.exists()) {
        setError("Please use the Teacher Portal. This is for students only.");
        setIsLoading(false);
        return;
      }
    ...
    <div className="min-h-screen bg-white flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="w-full max-w-md relative z-10">
        <div className="text-center mb-10">
          ...
          <h1 className="text-3xl font-bold text-gray-900 mb-2 tracking-tight">JuniorDream</h1>
          <p className="text-lg text-blue-600 font-bold">Student Portal</p>
        ...
        <div className="bg-white rounded-3xl shadow-xl shadow-gray-200/50 border border-gray-100 p-8 backdrop-blur-sm">
          <form onSubmit={handleLogin} className="space-y-5">
            ...
            <div>
              ...
              <div className={`relative transition-all duration-300 ${focusedField === 'email' ? 'scale-[1.02]' : ''}`}>
                ...
                <input id="email" type="email" value={email} onChange={(e) => setEmail(e.target.value)}
                  ...
                  placeholder="student@juniordream.com" required
            ...
            <button type="submit" disabled={isLoading}
              ...
              {isLoading ? (
                ...
                <><span>Sign In as Student</span><ArrowRight className="w-5 h-5" /></>
              )}
            </button>
          </form>
          ...
          <div className="mt-6 text-center">
            <p className="text-sm text-gray-500">
              Are you a teacher?{" "}
              <a href="https://teach.juniordream.com" className="text-purple-600 font-medium hover:text-purple-700 transition-colors">Teacher Login</a>
            </p>
          </div>
        </div>
        ...
        <div className="mt-8 text-center">
          <p className="text-xs text-gray-400">
            ...
            <a href="#" className="text-gray-500 hover:text-gray-700 underline transition-colors">Terms of Service</a>{" "}and{" "}
